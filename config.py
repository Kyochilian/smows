...
class Config:
    ...
    DEVICE = (
        "cuda:0" if os.environ.get("CUDA_VISIBLE_DEVICES") or True else "cpu"
    )  # Default to cuda:0, can be overridden
    ...
    def get_dataset_config(cls, dataset_name):
        ...
        if dataset_name not in cls.DATASETS:
            raise ValueError(
                f"Dataset '{dataset_name}' not found. Available datasets: {list(cls.DATASETS.keys())}"
            )
    ...
    def get_model_config(cls, config_name="default"):
        ...
        if config_name not in cls.MODEL_CONFIGS:
            raise ValueError(
                f"Model config '{config_name}' not found. Available configs: {list(cls.MODEL_CONFIGS.keys())}"
            )
    ...
    def get_training_config(cls, config_name="default"):
        ...
        if config_name not in cls.TRAINING:
            raise ValueError(
                f"Training config '{config_name}' not found. Available configs: {list(cls.TRAINING.keys())}"
            )
    ...
    def get_graph_config(cls, config_name="default"):
        ...
        if config_name not in cls.GRAPH_PARAMS:
            raise ValueError(
                f"Graph config '{config_name}' not found. Available configs: {list(cls.GRAPH_PARAMS.keys())}"
            )
        return cls.GRAPH_PARAMS[config_name]


def get_full_config(
    dataset="D1", model="default", training="default", graph="default", **overrides
):
    """
