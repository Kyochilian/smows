...
# clustering guidance
def distribution_loss(Q, P, mix_alpha=0.2, eps: float = 1e-12):
    """
    KL(P || Q_mix) style guidance for DEC-like clustering.

    The original implementation used the geometric mean in log-space:
      log_q = (log Q0 + log Q1 + log Q2) / 3
    which strongly penalizes cross-view disagreement and tends to wipe out
    minority clusters in imbalanced data.

    Here we use a convex mixture in probability-space:
      Q_mix = (1-a) * Q0 + a * (Q1 + Q2)/2
    which is much less brittle while still injecting multi-view signal.
    """
    q0, q1, q2 = Q[0], Q[1], Q[2]
    q_mix = (1.0 - mix_alpha) * q0 + mix_alpha * (q1 + q2) * 0.5
    return F.kl_div((q_mix + eps).log(), P, reduction='batchmean')
