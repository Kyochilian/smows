...
def load_human_lymph_node(name):
    ...
    label_path = f'./data/10X/{name}/label.npy'
    if os.path.exists(label_path):
        label = np.load(label_path)
    elif name == "Human_Lymph_Node_D1" and os.path.exists(f'./data/10X/{name}/D1_annotation_labels.csv'):
        # Fallback for D1 if npy is missing
        label = pd.read_csv(f'./data/10X/{name}/D1_annotation_labels.csv')['labels'].values
    else:
