...
# device config moved to main block
...
def pre_train(
    ...
    for epoch in range(num_epoch):
        ...
        loss_rec = (
            weight_list[0] * loss_ae1
            + weight_list[1] * loss_ae2
            + weight_list[2] * loss_ae3
            + weight_list[3] * loss_ae4
            + weight_list[4] * loss_x1
            + weight_list[5] * loss_x2
        )
        ...
        if epoch % 500 == 0:
            print(
                "Epoch: {:.0f}/{:.0f} ,loss:{:.8f}".format(epoch + 1, num_epoch, loss)
            )
...
def train(
    ...
    model.load_state_dict(
        torch.load(r"./pretrain/{}_pre_model.pkl".format(opt.name), map_location="cpu")
    )

    with torch.no_grad():
        Z, z1_tilde, z2_tilde, _, _, _, _, _, _, _ = model(
            x1, spatial_adj1, feature_adj1, x2, spatial_adj2, feature_adj2, Mt1, Mt2
        )
    ...
    if y is not None and best_metrics is not None:
        with open(
            os.path.join(result_dir, "{}_performance.csv".format(opt.name)), "a"
        ) as f:
            f.write(
                "seed:{}, lambda1:{}, lambda2:{}, spatial_k:{}, adj_k:{}, wieght_list:{}, ".format(
                    seed, lambda1, lambda2, spatial_K, adj_K, weight_list
                )
            )
    ...
    np.save(
        os.path.join(result_dir, "{}_{}_pre_label.npy".format(opt.name, run_idx)),
        y_pred,
    )
    np.save(
        os.path.join(result_dir, "{}_{}_laten.npy".format(opt.name, run_idx)),
        Z.data.cpu().numpy(),
    )
...
Examples:
  ...
  python main.py --config_name ablation_no_kl
    ...
    # Config selection
    parser.add_argument(
        "--dataset", type=str, default="D1", help="Dataset name from config.py"
    )
    parser.add_argument(
        "--config_name",
        type=str,
        default="default",
        help="Training config profile name (default, ablation_*, etc.)",
    )
    parser.add_argument(
        "--model_config",
        type=str,
        default="default",
        help="Model config profile name (default, smaller)",
    )

    # Optional overrides (if not provided, uses values from config.py)
    parser.add_argument(
        "--name", type=str, default=None, help="[Override] Short dataset name"
    )
    parser.add_argument(
        "--device", type=str, default=None, help="[Override] Device (cuda:0, cpu, etc.)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="[Override] Number of spatial neighbors",
    )
    parser.add_argument(
        "--adj_k", type=int, default=None, help="[Override] Number of feature neighbors"
    )
    parser.add_argument(
        "--lambda1", type=float, default=None, help="[Override] KL divergence weight"
    )
    parser.add_argument(
        "--lambda2", type=float, default=None, help="[Override] Consistency loss weight"
    )
    parser.add_argument(
        "--lr", type=float, default=None, help="[Override] Learning rate"
    )
    parser.add_argument(
        "--pretrain_epoch", type=int, default=None, help="[Override] Pretraining epochs"
    )
    parser.add_argument(
        "--train_epoch", type=int, default=None, help="[Override] Training epochs"
    )
    parser.add_argument(
        "--use_wandb", action="store_true", default=None, help="[Override] Enable WandB"
    )
    ...
    parser.add_argument(
        "--skip_pretrain", action="store_true", default=False, help="Skip pretraining"
    )
    ...
    try:
        ...
        print(
            f"✓ Loaded config profile: training='{args.config_name}', model='{args.model_config}' for dataset '{args.dataset}'"
        )
    ...
    # ========= 获取基因名列表 =========
    gene_names_view1 = adata_omics1.var_names.tolist()
    gene_names_view2 = adata_omics2.var_names.tolist()
    print(f"View1 (RNA) genes: {len(gene_names_view1)}")
    print(f"View2 (Protein/ADT) features: {len(gene_names_view2)}")
    adata_omics1, adata_omics2 = load_data(
        adata_omics1=adata_omics1,
        view1="RNA",
        adata_omics2=adata_omics2,
        view2="Protein",
        n_neighbors=opt.spatial_k,
        k=opt.adj_k,
    )
    ...
    # high-order graph
    Mt1 = process_adjacency_matrix(
        feature_adj1, "./pre_adj/{}/{}_Mt1.npy".format(opt.name, opt.name)
    )
    Mt2 = process_adjacency_matrix(
        feature_adj2, "./pre_adj/{}/{}_Mt2.npy".format(opt.name, opt.name)
    )
    ...
    print(f"GPU Memory after data loading: {torch.cuda.memory_allocated(device)/1024**3:.2f} GB allocated, {torch.cuda.memory_reserved(device)/1024**3:.2f} GB reserved")
    ...
    if opt.skip_pretrain:
        ...
        if os.path.exists(pretrain_model_path):
            print(
                "================================Skipping Pre_training...========================================="
            )
            print(f"Using existing pretrained model: {pretrain_model_path}")
        else:
            raise FileNotFoundError(
                f"Pretrained model not found: {pretrain_model_path}. Please run pretraining first or set --skip_pretrain to False."
            )
    else:
        print(
            "================================Pre_training...============================================"
        )
        # Clear memory before pretraining
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            print(f"Memory before pretrain: {torch.cuda.memory_allocated(device)/1024**3:.2f} GB allocated")
        ...
        z1_tilde, z2_tilde = pre_train(
            ...
            y=label,
            n_clusters=n_clusters,
            num_epoch=opt.pretrain_epoch,
            device=device,
            weight_list=cfg.get("weight_list", [1, 1, 1, 1, 1, 1]),
            lr=opt.lr,
            logger=logger,
            model_config=model_config,
            gene_names_view1=gene_names_view1,
            gene_names_view2=gene_names_view2,
        )
    ...
    for i in range(10):
        ...
        z1_tilde, z2_tilde, metrics = train(
            x1=data1,
            x2=data2,
            spatial_adj1=spatial_adj1,
            feature_adj1=feature_adj1,
    ...
    if all_metrics:
        df = pd.DataFrame(
            all_metrics, columns=["ACC", "F1", "NMI", "ARI", "AMI", "VMS", "FMS"]
        )
        ...
        if paper:
            for metric, paper_val in paper.items():
                if metric in df.columns:
                    ...
                    print(
                        f"{metric:>3s}: Ours={our_val:.4f} | Paper={paper_val:.4f} | Diff={diff:+.4f}"
                    )
        else:
